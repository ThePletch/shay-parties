  ...
  initializeDeleteButton() {
    ...
    deleteButton.on('click', () => {
      if (!this.element.data('persisted')) {
        ...
        this.element.find('input[type!="hidden"]').attr('disabled', (!currentlyMarkedForRemoval).toString());
...
class DynamicListManager {
  ...
  constructor(addButtonId: string) {
...
export function initializeDynamicListManagers() {
  $(() => {
