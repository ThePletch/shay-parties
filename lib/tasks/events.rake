...
namespace :events do
  ...
  task prewarm_header_images_and_wait: :environment do
    ...
    # AsyncAdapter only exposes immediate=; the reader lives on the scheduler.
    scheduler = adapter.instance_variable_get(:@scheduler)
    was_immediate = scheduler.immediate
    scheduler.immediate = true

    queued, skipped = queue_missing_header_image_transforms
    puts "Queued #{queued} header image transform job(s); skipped #{skipped} event(s) with existing variants."

    drain_async_active_jobs!(scheduler)
    puts "Finished waiting for header image transform job(s)."
  ensure
    scheduler.immediate = was_immediate unless was_immediate.nil?
  end
end
...
def drain_async_active_jobs!(scheduler)
